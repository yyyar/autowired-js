...
module.exports = class Autowired {
    ...
    validate(callback) {
        ...
        var self = this;
         ...
         var errs = self.unresolvedDeps;

         if (_.isEmpty(errs)) {
            return self.attachModules(callback);
     ...
     * Attach modules
     */
    attachModules(callback) {
        var self = this;
        _.each(self.config.modules, (m, k) => {
            var c = require(__dirname + '/ext/' + k);
            var i = new c(m.lib);
            i.process(self, function(){});
        });
        callback(null, this);
    }
